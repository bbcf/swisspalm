namespace :swisspalm do
  ...
  task :update_predictions, [:version] do |t, args|
...
#exit
 ...
 #       id = tab[0].split("|")[0].to_i
	id = pred.isoform_id
      ...
      if isoform	
        protein_id = h_isoforms[pred.isoform_id].protein_id
        pos = pred.pos #tab[1].to_i 
        if id != 0
          uniprot_ss=nil
 ...
 #            :cp_score => tab[3].to_f,
            ...
            #             :compatible_loc => (h_extracellular_prot[h_isoforms[pred.isoform_id].protein_id] or 
            #                                 (h_extracell_topo[h_isoforms[pred.isoform_id].protein_id] and h_extracell_topo[h_isoforms[pred.isoform_id].protein_id][pos]) or 
            #                                 h_d[[h_isoforms[pred.isoform_id].protein_id, pos]]) ? false : true
          }
          #  puts h_prediction.to_json
          pred.update_attribute(:psipred_ss, ((res['psipred_ss'][id]) ? res['psipred_ss'][id][pos] : nil)) 
          # prediction.save
        end
